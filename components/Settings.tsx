...
const Settings: React.FC<SettingsProps> = ({ onInstall, isInstallable, isInstalled }) => {
  ...
  const handleReset = async () => {
    if (!confirm('这将清除所有自定义网页工具、翻译历史并重置应用。确定吗？')) return;
    setIsResetting(true);
    localStorage.clear();
      ...
      <div className="space-y-6">
        ...
        <section className="bg-white/5 border border-white/10 rounded-2xl p-6 shadow-xl">
          <h3 className="text-lg font-semibold mb-4 text-indigo-400 flex items-center">
            <span className="mr-2">🤖</span> AI 翻译引擎
          </h3>
          
          <div className="space-y-4">
            <div>
              <label className="block text-xs font-bold text-white/40 uppercase mb-2 tracking-widest">提供商</label>
            ...
            {aiConfig.provider === AIProvider.OPENAI_COMPATIBLE && (
              <div className="space-y-4 pt-4 border-t border-white/5 animate-in fade-in slide-in-from-top-2 duration-300">
                <div>
                  <label className="block text-[10px] font-bold text-white/40 uppercase mb-1.5 ml-1">接口地址 (Base URL)</label>
                  <input 
                    type="text"
                ...
                <div>
                  <label className="block text-[10px] font-bold text-white/40 uppercase mb-1.5 ml-1">模型名称 (Model)</label>
                  <input 
                    type="text"
            ...
            {aiConfig.provider === AIProvider.GEMINI && (
              <div className="bg-indigo-500/5 border border-indigo-500/10 rounded-xl p-4 text-xs text-indigo-300/70">
                当前正使用系统内置的 Gemini API Key。请确保您的环境变量已正确配置。
              </div>
            )}
          </div>
        </section>
        ...
        <section className="bg-white/5 border border-white/10 rounded-2xl p-6">
          <div className="flex justify-between items-start mb-6">
            <div>
              <h3 className="text-lg font-semibold text-green-400">桌面安装</h3>
              <p className="text-sm text-white/60">将 MacTools 作为原生桌面应用运行。</p>
            </div>
            {isInstalled ? (
              ...
              <span className="text-xs text-white/30 italic">已是独立应用模式</span>
            )}
          </div>
        </section>
        ...
        {/* Reset Section */}
        <section className="bg-white/5 border border-white/10 rounded-2xl p-6">
          <h3 className="text-lg font-semibold mb-4 text-red-400">高级设置</h3>
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium">重置应用数据</p>
              <p className="text-xs text-white/40">清除所有数据，包括自定义 API Key 和网页工具。</p>
            </div>
            <button 
              onClick={handleReset}
